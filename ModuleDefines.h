...
struct HOTT_GAM_MSG {
	char start_byte;                      //#01 start char constant value 0x7c
	char gam_sensor_id;             	//#02 EAM sensort id. constat value 0x8d=GENRAL AIR MODULE
	char warning_beeps;                   //#03 1=A 2=B ... 0x1a=Z  0 = no alarm									  
	char sensor_id;             	        //#04 constant value 0xd0
	char alarm_invers1;                   //#05 alarm bitmask. Value is displayed inverted
										  //Bit#  Alarm field
										  // 0    all cell voltage
										  // 1    Battery 1
										  // 2    Battery 2
										  ...
										  // 6    rpm
										  // 7    Altitude
	char alarm_invers2;                   //#06 alarm bitmask. Value is displayed inverted
										  //Bit#  Alarm Field
	...
	char cell[6];				//#7 Volt Cell 1 (in 2 mV increments, 210 == 4.20 V)
	...
	uint16_t  Battery2;                   //#15 LSB battery 2 voltage LSB value. 0.1V steps. 50 = 5.5V only pos. voltages
										  //#16 MSB
	char temperature1;                    //#17 Temperature 1. Offset of 20. a value of 20 = 0°C
	char temperature2;                    //#18 Temperature 2. Offset of 20. a value of 20 = 0°C
	char fuel_percent;                    //#19 Fuel capacity in %. Values 0--100
	...
	uint16_t climbrate_L;                 //#26 climb rate in 0.01m/s. Value of 30000 = 0.00 m/s
										  //#27 MSB
	char climbrate3s;                     //#28 climb rate in m/3sec. Value of 120 = 0m/3sec
	...
	uint16_t speed;                       //#35 LSB (air?) speed in km/h(?) we are using ground speed here per default
										  //#36 MSB speed
	char min_cell_volt;                   //#37 minimum cell voltage in 2mV steps. 124 = 2,48V
	char min_cell_volt_num;               //#38 number of the cell with the lowest voltage
	uint16_t rpm2;                        //#39 LSB 2nd RPM in 10 RPM steps. 100 == 1000rpm
										  //#40 MSB
	char general_error_number;      	//#41 General Error Number (Voice Error == 12) TODO: more documentation
	char pressure_in_bar;                 //#42 High pressure up to 16bar. 0,1bar scale. 20 == 2.0bar
	char version;                         //#43 version number (Bytes 35 .43 new but not yet in the record in the display!)
	char stop_byte;                       //#44 stop char 0x7D
	char parity;                          //#45 CHECKSUM CRC/Parity (calculated dynamicaly)
};
...
struct gam_values {
	...
	char min_cell_volt_num;               //number of the cell with the lowest voltage
